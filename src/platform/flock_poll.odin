...
// the whole file — the same kernel-arbitrated, never-renamed, no-payload
// discipline as the Linux fcntl variant and the Windows LockFileEx one.
// The lock lives on the open file description, so a crashed holder
// releases it the moment its last descriptor closes: there is no pid to
// validate and no stale file to remove, which deletes the recovery race
// the previous O_EXCL+pid+remove design carried (a waiter's unlink could
// delete a fresh winner's lock between its liveness read and the remove,
// crowning two daemons for one project). The whole wait stays bounded —
// an unbounded block would let a wedged-but-alive holder wedge every
// future spawn.
//
...
LOCK_POLL_BUDGET_MS :: 30_000 // covers the winner's whole startup window
LOCK_POLL_SLICE_MS  :: 50

File_Lock :: struct {
	file: ^os.File,
}

file_lock_acquire :: proc(path: string, timeout_ms: i64 = LOCK_POLL_BUDGET_MS) -> (lock: File_Lock, ok: bool) {
	if timeout_ms <= 0 {
	...
	for {
		...
		time.sleep(LOCK_POLL_SLICE_MS * time.Millisecond)
	}
}
